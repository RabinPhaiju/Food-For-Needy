...
function calcMonthCalendar() {
    ...
    const currMonth = Array.from({ length: currentFullMonth.days_length },
        (_, i) => ({ day: i + 1, state: "currMonth" })
    );

    const nextMonth = Array.from({ length: currentFullMonth.days_length },
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            ...
            if (
                ...
                week.children[
                    j
                ].innerHTML = `${currentWeek[j].day}<img  id="todayLogo"  /> ${viewNote}`;
